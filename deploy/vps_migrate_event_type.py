...
print("=== Run migrate_event_type.sql ===")
run(f"cat {APP_DIR}/scripts/migrate_event_type.sql | docker exec -i app-postgres-1 psql -U pfm_user -d pfm_journeys", "migrate")

print("\n=== Verify ===")
col = pg("SELECT column_name FROM information_schema.columns WHERE table_name='events' AND column_name='event_type'")
print(f"     event_type column: {'OK' if col else 'MISSING'}")
